# Connector v1.1.5 deployment checkpoint
...
1. Require the compatible v1.1 MRL SQLite index first. Its `mrl_index` table
   ...
   values, and an age no greater than 35 days (35 × 24 hours) from its original
   `built_at`. Downloading the index again does not reset its age. Connector
   v1.1.5 does not require an index rebuild or schema change.
2. Download that pinned SQLite artifact by token and verify integrity, schema,
   ...
   reviewed v1.1.5 helper, including titles with curly apostrophes and en/em
   dashes.
3. Only after step 2 passes, require the complete macOS/Linux jobs to pass,
   then publish the v1.1.5 connector contract and FAQ by in-place overwrite of
   their existing pinned Feishu files. Never delete and recreate either file.
   Windows validation is deferred for v1.1.5 and is not a release gate.
...
5. Publish the public repository release last. The SKILL frontmatter, Git tag,
   and GitHub release must all identify v1.1.5.
